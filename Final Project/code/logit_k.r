...
for(i in 1:k){ #the k folds for each model
  test = filter(model1Shuffle, id >= (i-1)*numRows/k+1 & id <=i*numRows/k)
  train = anti_join(model1Shuffle, test, by="id")
  model1 <- glm(result~ poly (possession, 2) + touches + passes  + poly (shots_on_target , 2) + cards  + clearances, data= train, family=binomial,maxit=250)
  summary(model1)
  prob<-predict(object =model1,newdata=test,type = "response")
  pred<-ifelse(prob>=0.5,1,0)
  pred<-factor(pred,levels = c(0,1),order=TRUE)
...
for(i in 1:k){ #the k folds for each model
  test = filter(model1Shuffle, id >= (i-1)*numRows/k+1 & id <=i*numRows/k)
  train = anti_join(model1Shuffle, test, by="id")
  model1 <- glm(result~ poly (possession, 2) + touches + passes  + poly (shots_on_target , 2) + cards  + poly (clearances, 2), data=train,family=binomial,maxit=250)
  summary(model1)
  prob<-predict(object =model1,newdata=test,type = "response")
  pred<-ifelse(prob>=0.5,1,0)
  pred<-factor(pred,levels = c(0,1),order=TRUE)
...
for(i in 1:k){ #the k folds for each model
  test = filter(model1Shuffle, id >= (i-1)*numRows/k+1 & id <=i*numRows/k)
  train = anti_join(model1Shuffle, test, by="id")
  model1 <- glm(result~ poly (possession, 2) + touches + passes  + poly (shots_on_target , 3) + cards  + poly (clearances, 2), data= train, family=binomial,maxit=250)
  summary(model1)
  prob<-predict(object =model1,newdata=test,type = "response")
  pred<-ifelse(prob>=0.5,1,0)
  pred<-factor(pred,levels = c(0,1),order=TRUE)
...
for(i in 1:k){ #the k folds for each model
  test = filter(model1Shuffle, id >= (i-1)*numRows/k+1 & id <=i*numRows/k)
  train = anti_join(model1Shuffle, test, by="id")
  model1 <- glm(result~ poly (possession, 2) + touches + passes  + poly (shots_on_target , 2) + cards  + poly (clearances, 3), data= train, family=binomial,maxit=250)
  summary(model1)
  prob<-predict(object =model1,newdata=test,type = "response")
  pred<-ifelse(prob>=0.5,1,0)
  pred<-factor(pred,levels = c(0,1),order=TRUE)
...
for(i in 1:k){ #the k folds for each model
  test = filter(model1Shuffle, id >= (i-1)*numRows/k+1 & id <=i*numRows/k)
  train = anti_join(model1Shuffle, test, by="id")
  model1 <- glm(result~ poly (possession, 2) + touches + passes  + poly (shots_on_target , 4) + cards  + poly (clearances, 2), data= train, family=binomial,maxit=250)
  summary(model1)
  prob<-predict(object =model1,newdata=test,type = "response")
  pred<-ifelse(prob>=0.5,1,0)
  pred<-factor(pred,levels = c(0,1),order=TRUE)
  ...
  errors[10,i] = (f[1,1] +　f[2,2])/nrow(test)
}
...
p1 <-ggplot(data = faithBest, aes(x = x, y=avgAcc.k))+ geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin = avgAcc.k-se, ymax = avgAcc.k +se))+labs(x="Model Complexity")+ggtitle("avgCp with model complexity")+theme(plot.title = element_text(hjust = 0.5))
p1
ggsave("avgAcc_with_se.png",p1,width = 12,height = 9)

p2 <- ggplot(data = faithBest, aes(x = x, y=avgAcc.k))+ geom_point()+
  geom_line()+labs(x="Model Complexity")+ggtitle("avgCp with model complexity")+theme(plot.title = element_text(hjust = 0.5))
p2
